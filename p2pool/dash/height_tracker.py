...
@defer.inlineCallbacks
def get_height_rel_highest_func(aerisd, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(aerisd.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield aerisd.rpc_getblock('%x' % (block_hash,))
